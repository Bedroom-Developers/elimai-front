  ...
  const { mutate: login, isLoading } = useMutation({
          ...
          rGetGames()
            .then((games) => {
              const active = games.find((game) => {
                if (
                  dayjs(game.event_date).get("date") == dayjs().get("date") &&
                  dayjs(game.event_date).get("month") == dayjs().get("month")
                ) {
                  return true;
                }
                return false;
    ...
    onError: (e) => {
      ...
      showErrorNotification({
        title: t("errors.auth.login.title"),
        message: t("errors.auth.login.message"),
      });
    },
  });
      ...
      <Stack px={20} py={10} miw={350} w={"100%"} gap={10}>
        ...
        <Text size="sm" c={"slate.6"}>
          {t.rich("auth.login.restore", {
            a: (chunk) => (
              <Link className="link" href={"/restore"}>
                {chunk}
              </Link>
            ),
          })}
