...
export const BuyTicketForm = ({
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<{ tel: string; count: string; FIO: string }>();
  const {
    mutate,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Stack p={10}>
        <TextInput
          required
          label={t("buy.form.fio")}
          error={errors["FIO"]?.message}
          {...register("FIO", { required: t("errors.required") })}
        ...
        <Controller
          ...
          rules={{
            ...
            validate: (value) => {
              ...
              return digits.length === 11 || t("errors.required");
            },
          }}
