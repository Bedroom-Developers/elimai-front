...
export const AdminGamesTable = () => {
  ...
  const rows = games.map((element, idx) => (
    <Table.Tr key={element.id}>
      <Table.Td ta={"center"}>
        {dayjs(element.event_date).locale("ru").format("MMMM D,YYYY HH:mm")}
      </Table.Td>
      <Table.Td ta={"center"}>{element.name_ru}</Table.Td>
      <Table.Td ta={"center"}>{element.name_kz}</Table.Td>
      <Table.Td ta={"center"}>{element.status}</Table.Td>
      <Table.Td ta={"center"}>
...
const CreateAdminTicketBtn = ({ game }: { game: Game }) => {
  ...
  const {
    ...
    mutationKey: [`admin-ticket ${game.id}`],
    mutationFn: rCreateAdminTicket,
    onSuccess: (data) => {
      createTicket(
        [
          {
            code: data.code,
            date: dayjs(game.event_date).format("YYYY-MM-DD HH:mm"),
            name_ru: game.name_ru,
            name_kz: game.name_kz,
            status: data.status,
          },
        ],
        true
      ...
      onClick={() =>
        create({
          data: {
            ...
            email: "admin",
            telephone: "admin",
            will_deactivate_at: dayjs(game.event_date).format("YYYY-MM-DD"),
            status: "Active",
            psign: "none",
...
const GetExcelTicketBtn = ({ game }: { game: Game }) => {
  const {
    ...
    mutationKey: [`excel ${game.id}`],
    mutationFn: rGetGameExcel,
    onSuccess: (data) => {
      const url = window.URL.createObjectURL(data);
      const a = document.createElement("a");
      a.href = url;
      a.download = `${game.name_ru}-${dayjs(game.event_date).format(
        "YYYY-MM-DD HH:mm"
      )}.xlsx`;
...
const GetExcelSubBtn = () => {
  const {
    ...
    mutationKey: [`excel sub`],
    mutationFn: rGetSubExcel,
    onSuccess: (data) => {
      const url = window.URL.createObjectURL(data);
      const a = document.createElement("a");
      a.href = url;
  ...
  return (
    <Button bg={"slate.7"} loading={isLoading} onClick={() => getExcel()}>
      Отчет Абонементы
    </Button>
  );
};
const CreateAdminSubBtn = () => {
  ...
  const {
    ...
    mutationKey: [`admin-ticketsub`],
    mutationFn: rCreateAdminSub,
    onSuccess: (data) => {
      createSub(data.code);
    },
    onError: (e) => {
      ...
      onClick={() =>
        create({
          data: {
            ...
            email: "admin",
            telephone: "admin",
            will_deactivate_at: dayjs(new Date()).format("YYYY-MM-DD"),
            status: "Active",
            psign: "none",
            code: "aboniment-" + Date.now(),
          },
        })
      }
    >
