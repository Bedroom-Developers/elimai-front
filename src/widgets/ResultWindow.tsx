  ...
  return (
    <Box mt={20} mih={"50svh"}>
      <Center w={"100%"} h={"100%"} pos={"relative"}>
        {isFetching ? (
          <Loader color="elimai.6" />
        ) : !error ? (
          type == "Aboniment" ? (
            <SuccessSubView tickets={tickets} />
          ) : (
  ...
  return (
    <Stack p={{ sm: 10, md: 0 }}>
      <Alert
        ...
        variant="filled"
        color="elimai.2"
        title={t("result.success.tickets.message")}
      >
        <span>
          {t("result.success.tickets.count", { count: tickets?.length })}
        </span>
        <ul>
      ...
      <Group grow>
        ...
        {gameId && (
          <BuyTicketBtn again variant="outline" gameId={parseInt(gameId)} />
        )}
      </Group>
  ...
  return (
    <Stack p={{ sm: 10, md: 0 }}>
      <Alert
        ...
        variant="filled"
        color="elimai.2"
        title={t("result.success.sub.message")}
      >
        <ul>
  ...
  return (
    <Stack justify="center" align="center">
      ...
      <Title ta={"center"} order={3} c={"slate.4"}>
        {t("result.notStarted.title")}{" "}
        <button disabled={timer !== 0} onClick={onClick}>
          <Text fw={"bold"} size="lg" c={timer > 0 ? "slate.4" : "elimai.6"}>
            {t("result.notStarted.timer.text")}{" "}
